...
## AIM:
To write a Java program using abstract classes and method overriding where different signal agents decode signals based on either prime-number summation (PrimeAgent) or mirror/balanced pattern detection (MirrorAgent).

## ALGORITHM :
1.	Start the program.
2.	Import the necessary package 'java.util'
3.	Define an abstract class SignalAgent with an abstract method decodeSignal(int[] signal) to be implemented by subclasses.
4. Create a subclass PrimeAgent that overrides the method to sum all prime numbers in the signal array.
5. Create another subclass MirrorAgent that checks whether the signal array is symmetrical (mirrored) by comparing elements from both ends.
6. In the main() method, read the signal array and the agent type (1 for PrimeAgent, otherwise MirrorAgent).
7. Use runtime polymorphism to assign the appropriate agent object, call decodeSignal(), and display the output based on the agent type.
...
<img width="429" height="288" alt="image" src="https://github.com/user-attachments/assets/2d7b8a7b-2d68-4c2e-8f0e-a1ce074e76dc" />


## RESULT:
Thus, the program successfully demonstrated abstraction and runtime polymorphism by decoding signals differently using PrimeAgent and MirrorAgent, producing correct outputs for prime summation and mirror checking.
